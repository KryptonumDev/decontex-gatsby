...
import {
  ...
  Spain,
  Switzerland
} from '../childrens/map/countries'
import { Map } from './../childrens/map/map'
// import Map from './../../resources/map.png'

export default function InteractiveMap({
  data: { sectionTitle, text, country, next }
}) {
  ...
  useEffect(() => {
    setCord({
      x: mapEl.current.offsetLeft,
      y:
        mapEl.current.offsetTop +
        mapEl.current.clientHeight / 2
    })
    setTimeout(() => {
      setIsOpened(true)
    }, 100)
  ...
  const mobileOpen = (num) => {
    let currentItem = country.filter(
      (el) => el.countryCode === num
    )
  ...
  const open = (num) => {
    ...
    let x =
      window.event.pageX - itemEl.current.clientWidth / 2
    let y =
      window.event.pageY - itemEl.current.clientHeight / 2

    if (x < 0) {
      x = 0
    } else if (
      x + itemEl.current.clientWidth >
      window.innerWidth
    ) {
      ...
      <LocContainer>
        ...
        <Plate
              ...
              <div className='mobile-flex'>
                ...
                <div>
                  {el.countryPhone.map((innerEl) => (
                    <a
                      tabIndex={
                        whichOpened === el.countryCode &&
                        isOpened
                          ? '0'
                          : '-1'
                      }
                      href={'tel:' + innerEl.text}>
                      <svg
                        width='26'
                        height='26'
                        viewBox='0 0 26 26'
          ...
          <div className='buttons'>
            <button
              className='button'
              aria-label='previously country'
              onClick={() => {
                mobileOpen(
                  openedIndex === 0
                    ? country[country.length - 1]
                        .countryCode
                    : country[openedIndex - 1].countryCode
                )
              }}>
              <svg
                width='40'
                ...
                xmlns='http://www.w3.org/2000/svg'>
                <path
                  d='M20 40L0 20L20 0L22.625 2.625L7.125 18.125H40V21.875H7.125L22.625 37.375L20 40Z'
                  fill='#F3F3F3'
                />
              </svg>
            </button>
...
const Plate = styled.div`
  ...
  transform: translateX(${(props) => props.cord.x}px)
    translateY(${(props) => props.cord.y}px);
...
const BrownText = styled.div`
  width: 100%;
  padding: clamp(12px, ${(24 / 768) * 100}vw, 44px)
    clamp(16px, ${(40 / 768) * 100}vw, 64px);
